...
pretrain_optimizer = optim.RMSprop(lstm.parameters(), lr=config['pretrain_learning_rate'])
...
for epoch in range(config['pretrain_epoches']):
    for i, data in enumerate(train_loader):
        ...
        predicted_shot = generator(out_shot)
        out_shot = out_shot.view(config['batch_size'], 1,-1)
        print(out_shot.size())
        print(predicted_shot[0].size())
        loss = mse(predicted_shot[0], out_shot)
        loss.backward()
...
for epoch in range(config['gan_epoches']):
    for i, (data, sample) in enumerate(zip(train_loader, sample_loader)):
        ...
        _, sample = sample
        sample = sample.cuda()
        sample = sample.view(1, config['batch_size'], -1)
        ...
        out_shot = out_shot.view(1, config['batch_size'], -1)
